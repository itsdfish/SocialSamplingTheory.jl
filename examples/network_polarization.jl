...
function initialize(; n_agents, γ = 20, w = 0.50)
    space = GridSpace((n_agents, n_agents); periodic = true)
    model = ABM(SocialAgent, space; scheduler = Schedulers.Randomly())
    id = 1
    for _ = 1:n_agents, _ = 1:n_agents
        α, β = rand_parms()
        public_attitude = mean(Beta(α, β))
        agent = SocialAgent(id, (1, 1); α, β, γ = 20, w = 0.50, public_attitude)
...
model = initialize(; n_agents = 100, γ = 20, w = 0.50)
